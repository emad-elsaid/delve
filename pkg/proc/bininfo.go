...
func (bi *BinaryInfo) openSeparateDebugInfo(image *Image, exe *elf.File, debugInfoDirectories []string) (*os.File, *elf.File, error) {
	...
	if debugFilePath == "" {
		...
		if debugFilePath != "" {
			// CRC check
			computedCRC, ok := gnuDebugLinkCRC(debugFilePath)
			if ok && crc != computedCRC {
...
var gnuDebugLinkCRCTable = [256]uint32{
...
// gnuDebugLinkCRC calculates the CRC for gnu_debuglink as specified in:
//
//	https://sourceware.org/gdb/onlinedocs/gdb/Separate-Debug-Files.html
func gnuDebugLinkCRC(debugFilePath string) (uint32, bool) {
	...
	crc = ^crc & 0xffffffff
	for _, ch := range buf {
		crc = gnuDebugLinkCRCTable[(crc^uint32(ch))&0xff] ^ (crc >> 8)
	}
	return ^crc & 0xffffffff, true
}
